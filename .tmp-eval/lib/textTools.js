...
exports.prepareHumanizeInput = prepareHumanizeInput;
exports.humanizeText = humanizeText;
...
exports.estimateHumanScore = estimateHumanScore;
exports.buildProcessingNotes = buildProcessingNotes;
exports.humanizeTextRetry = humanizeTextRetry;
exports.countWords = countWords;
exports.evaluateHumanizeCandidate = evaluateHumanizeCandidate;
const replacementLibrary_1 = require("@/lib/replacementLibrary");
const starterCleanupLibrary_1 = require("@/lib/starterCleanupLibrary");
const essayPatternLibrary_1 = require("@/lib/essayPatternLibrary");
const AI_PHRASES = [
    /\bin conclusion\b/gi,
    /\bto conclude\b/gi,
    /\bmoreover\b/gi,
...
function lightPostProcess(text) {
...
function normalizeMeaningNeutralScaffold(text) {
    return normalizeWhitespace(text
...
function protectMeaningTokens(text) {
...
function restoreMeaningTokens(text, protectedTokens) {
    let out = text;
...
function softenGenericAiScaffold(text) {
 ...
 */
function applyWordReplacements(text, strength, options) {
    var _a, _b, _c;
    let out = text;
    const shouldSkipPhraseReplacement = (replacement) => /^(overall|clearly,?|helps a lot)$/i.test(replacement.trim());
    const shouldSkipWordReplacement = (replacement) => /^(key|solid|dependable)$/i.test(replacement.trim());
    const essayLimitBase = strength === "light" ? 3 :
        strength === "medium" ? 6 :
            10;
    const phraseLimitBase = strength === "light" ? 4 :
        strength === "medium" ? 10 :
            18;
    const wordLimitBase = strength === "light" ? 3 :
        strength === "medium" ? 6 :
            8;
    const essayLimit = Math.max(0, essayLimitBase + ((_a = options === null || options === void 0 ? void 0 : options.phraseBoost) !== null && _a !== void 0 ? _a : 0));
    const phraseLimit = Math.max(0, phraseLimitBase + ((_b = options === null || options === void 0 ? void 0 : options.phraseBoost) !== null && _b !== void 0 ? _b : 0));
    const wordLimit = Math.max(0, wordLimitBase + ((_c = options === null || options === void 0 ? void 0 : options.wordBoost) !== null && _c !== void 0 ? _c : 0));
    let essayApplied = 0;
    for (const [pattern, replacement] of essayPatternLibrary_1.ESSAY_PATTERN_REPLACEMENTS) {
        if (essayApplied >= essayLimit)
            break;
        if (shouldSkipPhraseReplacement(replacement))
            continue;
    ...
    let phraseApplied = 0;
    for (const [pattern, replacement] of replacementLibrary_1.PHRASE_REPLACEMENTS) {
        if (phraseApplied >= phraseLimit)
            break;
        if (shouldSkipPhraseReplacement(replacement))
            continue;
        const next = out.replace(pattern, replacement);
    ...
    let wordApplied = 0;
    for (const [pattern, replacement] of replacementLibrary_1.WORD_REPLACEMENTS) {
        if (wordApplied >= wordLimit)
            break;
        if (shouldSkipWordReplacement(replacement))
            continue;
        const next = out.replace(pattern, replacement);
        if (next !== out) {
            out = next;
...
function rotateClause(sentence) {
...
function breakLongSentence(sentence) {
    ...
    const safeConnectorMatch = sentence.match(/^(.*?)(\s+(?:but|yet|so|because|although|while)\s+.+)$/i);
    if (safeConnectorMatch) {
        const firstHalf = (_a = safeConnectorMatch[1]) === null || _a === void 0 ? void 0 : _a.trim().replace(/[,.!?;:]+$/, "");
        const secondHalf = (_b = safeConnectorMatch[2]) === null || _b === void 0 ? void 0 : _b.trim();
        if (firstHalf &&
            secondHalf &&
...
function looksLikeCompleteClause(text) {
...
function combineShortSentences(sentences) {
    const result = [];
...
function hasSafeStandaloneOpening(sentence) {
    var _a;
    const opener = ((_a = sentence.trim().split(/\s+/)[0]) === null || _a === void 0 ? void 0 : _a.toLowerCase().replace(/[^\w']/g, "")) || "";
...
function startsGeneric(sentence) {
    return /^(It|This|There)\b/.test(sentence.trim());
}
function sentenceLooksStandalone(sentence) {
    const trimmed = sentence.trim();
    if (!trimmed)
        return false;
    return (countWords(trimmed) >= 7 &&
        !/^(Because|Although|While|And|But|So|Which)\b/.test(trimmed));
}
function restructureSentenceSequence(sentences, reorderLevel) {
    if (reorderLevel === 0 || sentences.length < 2) {
        return sentences;
    }
    const reordered = [...sentences];
    if (startsGeneric(reordered[0]) &&
...
function reshapeParagraphStructure(paragraph, strength, mode, tone, reorderLevel = 0) {
...
function startsWithTransition(sentence) {
    return /^(However|Also|Still|As a result|For that reason|At the same time|In practice|Overall)\b/i.test(sentence.trim());
}
function isFormalStudentMode(mode) {
    return ["school", "report", "thesis", "research", "proposal"].includes(mode);
}
function isKeyPointSentence(sentence, mode) {
    const trimmed = cleanArtifacts(sentence);
    if (/\b(main change is|better argument|early feedback suggests|more useful than|showed me that|taught me that)\b/i.test(trimmed)) {
        return true;
    }
...
function isSetupSentence(sentence) {
    const trimmed = cleanArtifacts(sentence);
    return /\b(was designed to|was introduced to|are often defended as|may reduce some visible differences|the process felt|the training program was designed)\b/i.test(trimmed);
}
function mergeExplanationPairs(sentences) {
    var _a;
    const merged = [];
    for (let index = 0; index < sentences.length; index++) {
        const current = cleanArtifacts(sentences[index]);
        const next = cleanArtifacts((_a = sentences[index + 1]) !== null && _a !== void 0 ? _a : "");
        if (next &&
            /^I learned\b/i.test(current) &&
            /^This showed me that\b/i.test(next) &&
            countWords(current) + countWords(next) <= 34) {
            merged.push(cleanArtifacts(`${current.replace(/[.!?]+$/, "")} because ${lowerFirst(next
        ...
        if (next &&
            ...
            merged.push(cleanArtifacts(`${current.replace(/[.!?]+$/, "")}, but ${lowerFirst(next)}`));
            index++;
            continue;
        }
        if (next &&
...
function reshapeFormalStudentProgression(sentences, mode, strength) {
    if (!isFormalStudentMode(mode) || sentences.length < 2) {
        return sentences;
    }
    const reordered = mergeExplanationPairs(sentences);
    const keyIndex = reordered.findIndex((sentence) => isKeyPointSentence(sentence, mode));
    if (keyIndex > 0) {
    ...
    const contrastIndex = reordered.findIndex((sentence) => /^(Yet|However|Still|At first|For that reason)\b/i.test(sentence));
    if (strength === "strong" &&
        setupIndex === 0 &&
        contrastIndex > 0 &&
...
function moveDependentClause(sentence, preferFront) {
    var _a, _b, _c, _d, _e, _f;
    const trimmed = cleanArtifacts(sentence);
    if (preferFront) {
        const trailing = trimmed.match(/^(.*?),\s*(because|although|while|when|if|since)\s+(.+)$/i);
        if (trailing) {
            const mainClause = (_a = trailing[1]) === null || _a === void 0 ? void 0 : _a.trim();
            const marker = (_b = trailing[2]) === null || _b === void 0 ? void 0 : _b.trim();
            const dependent = (_c = trailing[3]) === null || _c === void 0 ? void 0 : _c.replace(/[.!?]+$/, "").trim();
            if (mainClause &&
                dependent &&
                countWords(mainClause) >= 5 &&
                countWords(dependent) >= 4) {
                return cleanArtifacts(`${capitalize(marker)} ${dependent}, ${lowerFirst(mainClause)}.`);
            }
        }
    }
    const leading = trimmed.match(/^(Because|Although|While|When|If|Since)\s+(.+?),\s+(.+)$/i);
    if (leading) {
        const marker = (_d = leading[1]) === null || _d === void 0 ? void 0 : _d.trim().toLowerCase();
        const dependent = (_e = leading[2]) === null || _e === void 0 ? void 0 : _e.trim();
        const mainClause = (_f = leading[3]) === null || _f === void 0 ? void 0 : _f.replace(/[.!?]+$/, "").trim();
        if (marker &&
            dependent &&
            mainClause &&
            countWords(mainClause) >= 5 &&
...
function locallyRecomposeSentence(sentence, mode, strength, genericAiSafe = false) {
    ...
    out = out
        ...
        .replace(/^School uniforms are often defended as a way to create equality,\s*but that argument does not fully reflect what students actually experience\.$/i, "Uniforms are often defended in the name of equality, but that claim does not fully match what students actually experience.")
        .replace(/^Wearing the same clothes may reduce some visible differences,\s*yet it does not automatically remove social pressure or economic inequality\.$/i, "The same clothes may hide some visible differences, but they do not automatically remove social pressure or economic inequality.")
        .replace(/^A better argument for uniforms is that they can simplify daily routines for families and reduce distractions in some school environments,\s*although even that benefit depends on how the policy is applied\.$/i, "A better argument for uniforms is that they can make daily routines easier for families and can, in some school environments, reduce distractions, though that benefit still depends on how the policy is applied.");
    if (mode === "report") {
        out = out
            .replace(/^The training program was designed to support newly promoted team leads during their first three months in role\.$/i, "The training program was designed to help newly promoted team leads through their first three months in role.")
            .replace(/^Rather than focusing only on theory,\s*the program combines short workshops,\s*peer discussion,\s*and scenario-based exercises that reflect common management challenges\.$/i, "Rather than focusing only on theory, the program uses short workshops, peer discussion, and scenario-based exercises based on common management challenges.")
            .replace(/^Participants are expected to practice difficult conversations,\s*review examples of unclear delegation,\s*and document how they would respond in realistic situations\.$/i, "Participants are expected to do three things: practice difficult conversations, review unclear delegation examples, and document how they would respond in realistic situations.")
            .replace(/^Early feedback suggests that the practical format is more useful than longer presentation-style sessions because it allows participants to test ideas and receive immediate input from others\.$/i, "Early feedback suggests the practical format is more useful than longer presentation-style sessions because participants can test ideas and receive immediate input from others.")
            .replace(/^At the same time,\s*the pilot also showed that time pressure remains a problem for some teams,\s*particularly when workshops are scheduled during busy reporting periods\.$/i, "The pilot also showed that some teams still struggle with time pressure, especially when workshops are scheduled during busy reporting periods.")
            .replace(/^For that reason,\s*the next version of the program will likely keep the same content while adjusting delivery times and shortening a few exercises\.$/i, "For that reason, the next version will likely keep the same content but adjust delivery times and shorten a few exercises.")
            .replace(/^The policy was introduced to reduce preventable delays in internal approvals and to improve the consistency of project documentation across departments\.$/i, "The policy was introduced to cut preventable delays in internal approvals and make project documentation more consistent across departments.")
            .replace(/^In practice,\s*the main change is that teams must submit a clearer project summary before requesting budget review\.$/i, "In practice, teams now need to submit a clearer project summary before asking for budget review.")
            .replace(/^This (additional|extra) step may appear minor,\s*but it is intended to reduce back-and-forth communication later in the process and to make approval decisions easier to track\.$/i, "That extra step may seem minor, but it is meant to cut down on back-and-forth later and make approval decisions easier to track.");
    }
    return cleanArtifacts(out);
}
function refineHumanizedOutput(text) {
    return cleanArtifacts(text
...
function chooseLeadSentenceIndex(sentences) {
    let bestIndex = 0;
    let bestScore = Number.NEGATIVE_INFINITY;
    for (let index = 0; index < sentences.length; index++) {
        const sentence = sentences[index];
        const words = countWords(sentence);
        if (!sentenceLooksStandalone(sentence))
            continue;
        let score = Math.min(words, 22);
        if (!startsGeneric(sentence))
            score += 6;
        if (!startsWithTransition(sentence))
            score += 3;
...
function moveLeadSentenceFirst(sentences) {
...
function moveTransitionSentenceLater(sentences) {
...
function applyParagraphStrategy(paragraph, strategy, strength, mode, tone, reorderLevel) {
    ...
    if (strategy === "reordered_emphasis" || strategy === "structured_strong") {
        sentences = restructureSentenceSequence(moveTransitionSentenceLater(sentences), Math.max(reorderLevel, 1));
    }
 ...
 */
function diversifyOpeners(sentences) {
    const used = new Set();
    ...
    return sentences.map((sentence, index) => {
        ...
        if (index % 2 === 0 && words.length >= 10) {
            ...
            const rotatedOpener = (_a = rotatedWords[0]) === null || _a === void 0 ? void 0 : _a.toLowerCase().replace(/[^\w']/g, "");
            const rotatedPair = rotatedWords.slice(0, 2).join(" ").toLowerCase();
            if (rotatedWords.length >= 6 &&
                rotatedOpener &&
 ...
 * - countWords(text)
 */
function varyRhythm(text, strength, mode, tone) {
    ...
    sentences = sentences.map((sentence, index) => {
        ...
        if (strength === "strong" && countWords(out) > 20) {
            const broken = breakLongSentence(out);
            const parts = splitSentences(broken);
            const validSplit = parts.length === 2 &&
                countWords(parts[0]) >= 6 &&
                countWords(parts[1]) >= 6;
            if (validSplit) {
                out = broken;
...
function cleanupHumanStyle(text) {
    return cleanArtifacts(text
...
function countAiPhrases(text) {
    ...
    return patterns.reduce((total, pattern) => {
        ...
        return total + ((_a = matches === null || matches === void 0 ? void 0 : matches.length) !== null && _a !== void 0 ? _a : 0);
    }, 0);
}
function analyzeSourceProfile(text) {
    const sentences = splitSentences(text);
    const starts = sentences.map((sentence) => sentence.trim().split(/\s+/).slice(0, 2).join(" ").toLowerCase());
    const repeatedOpeners = starts.length - new Set(starts).size;
    const aiPhraseHits = countAiPhrases(text);
    const genericLeadHits = (text.match(/\b(In today's world|Moreover|Furthermore|Additionally|Overall|It is important to note that)\b/gi) || []).length;
    return {
        aiPhraseHits,
...
function paragraphRhythmPenalty(text) {
...
function rewriteByTone(text, tone) {
    let out = text;
    if (tone === "formal") {
        out = out
...
function rewriteByMode(text, mode) {
    let out = text;
...
function meaningRetentionScore(input, output) {
    ...
    const lengthRatio = inputWords === 0
        ? 1
        : Math.min(outputWords / inputWords, inputWords / Math.max(outputWords, 1));
    const sharedSpanRatio = inputWords === 0 ? 0 : Math.min(longestSpan / Math.max(inputWords, 1), 1);
    return Math.max(0, Math.min(100, lexical * 62 +
        lengthRatio * 28 +
        sharedSpanRatio * 10));
}
function openerSimilarity(input, output) {
...
function sentenceProgressionRetention(input, output) {
    const sourceSentences = splitSentences(input).map((sentence) => cleanArtifacts(sentence).toLowerCase());
    const rewrittenSentences = splitSentences(output).map((sentence) => cleanArtifacts(sentence).toLowerCase());
 ...
 */
function overallSimilarity(input, output) {
    ...
    const sharedSpanPenalty = Math.min(longestSpan / 10, 1) * 0.32;
    // Extra penalty if copied spans are very long
    const copiedBlockPenalty = longestSpan >= 14 ? 0.12 :
        longestSpan >= 10 ? 0.07 :
            longestSpan >= 7 ? 0.03 :
                0;
    return Math.min(1, lexical * 0.34 +
        opener * 0.20 +
        rhythmPenalty +
        sharedSpanPenalty +
        copiedBlockPenalty);
 ...
 */
function aggressiveSecondPass(text, mode, tone) {
    const sentences = splitSentences(text);
 ...
 */
function repairSentenceBoundaries(text) {
    return cleanArtifacts(text
        .replace(/([a-z0-9\)])\s+([A-Z])/g, "$1. $2")
        .replace(/\.\s*\./g, ".")
        .replace(/\s{2,}/g, " "));
}
function normalizeStrongSentence(sentence) {
    return cleanArtifacts(sentence
...
function looksLikeListContinuation(sentence) {
...
function previousCanAbsorbContinuation(previous) {
    return /\b(combines|include|includes|including|uses|offers|expects|expected|practice|practices|review|reviews|covers|covered|supports|supported|involves|involved)\b/i.test(previous);
}
function shouldMergeWithPrevious(previous, current) {
    if (!previous || !current)
        return false;
    const cleanedCurrent = current.trim();
    if (/^(Because|Although|While|If|Since)\b/.test(cleanedCurrent) &&
        countWords(previous) <= 24 &&
        countWords(previous) + countWords(cleanedCurrent) <= 34) {
        return true;
    }
    if (!looksLikeCompleteClause(cleanedCurrent) &&
        countWords(previous) <= 22 &&
        countWords(previous) + countWords(cleanedCurrent) <= 30) {
        return true;
    }
    if (looksLikeListContinuation(cleanedCurrent) &&
        previousCanAbsorbContinuation(previous) &&
        countWords(previous) + countWords(cleanedCurrent) <= 34) {
        return true;
    }
    if (/^This is especially true when\b/i.test(cleanedCurrent) &&
        countWords(previous) <= 22 &&
...
function polishStrongCandidate(text) {
    const paragraphs = splitParagraphs(text);
    const polishedParagraphs = paragraphs.map((paragraph) => {
        ...
        for (const rawSentence of sentences) {
            ...
            if (previous && shouldMergeWithPrevious(previous, normalized)) {
                polished[polished.length - 1] = cleanArtifacts(`${previous.replace(/[.!?]+$/, "")}, ${lowerFirst(normalized)}`);
                continue;
            }
            if (previous &&
...
function smoothStrongParagraphFlow(paragraphs) {
    return paragraphs.map((paragraph) => {
        ...
        for (let index = 0; index < sentences.length; index++) {
            ...
            if (/^This (allows|helps|makes|keeps|reduces|improves|supports)\b/i.test(current) &&
                countWords(previous) <= 24 &&
                countWords(previous) + countWords(current) <= 34) {
                smoothed[smoothed.length - 1] = cleanArtifacts(`${previous.replace(/[.!?]+$/, "")}, ${lowerFirst(current)}`);
                continue;
            }
            smoothed.push(current);
        }
 ...
 */
function rewriteCore(inputText, tone, mode, strength) {
...
function prepareHumanizeInput(inputText) {
...
function humanizeText(inputText, tone = "natural", mode = "standard", strength = "medium") {
    const sourceProfile = analyzeSourceProfile(inputText);
    const candidates = generateRewriteCandidates(inputText, tone, mode, strength);
    let best = pickBestRewriteCandidate(candidates, strength);
    if (!best.score.meaningSafe && sourceProfile.genericAiLike) {
        const preservationRetry = preservationFocusedRetry(inputText, tone, mode, strength);
        const preservationScore = scoreRewriteCandidate(inputText, preservationRetry, strength, sourceProfile);
    ...
    if (feelsLightlyEditedAiText(inputText, best.outputText, strength)) {
        const restructureRetry = targetedRestructureRetry(inputText, best.outputText, tone, mode, strength === "light" ? "medium" : strength);
        const restructureScore = scoreRewriteCandidate(inputText, restructureRetry, strength, sourceProfile);
        if (restructureScore.meaningSafe &&
            ...
            const deeperCandidates = generateRewriteCandidates(inputText, tone, mode, strength === "light" ? "medium" : "strong");
...
function paraphraseText(inputText, tone = "natural", mode = "standard", strength = "medium") {
...
function rewriteText(inputText, tone = "natural", mode = "standard", strength = "medium") {
    return humanizeText(inputText, tone, mode, strength);
}
function improveText(inputText, tone = "natural", mode = "standard", strength = "medium") {
    const effectiveStrength = strength === "strong" ? "medium" : strength;
    return rewriteCore(inputText, tone, mode, effectiveStrength);
}
function expandText(inputText, tone = "natural", mode = "standard", strength = "medium") {
    const effectiveStrength = strength === "light" ? "medium" : strength;
    return rewriteCore(inputText, tone, mode, effectiveStrength);
}
function shortenText(inputText, tone = "natural", mode = "standard", strength = "medium") {
    const original = normalizeWhitespace(inputText);
...
function grammarText(inputText) {
...
function estimateHumanScore(inputText, outputText, tool, strength = "medium") {
    ...
    if (tool === "shorten")
        base = 84;
    const strengthBoost = strength === "light" ? -2 :
        strength === "medium" ? 0 :
            3;
...
function buildProcessingNotes(tool, tone, mode, strength = "medium") {
    const notes = [
        `Tool applied: ${tool}`,
        `Tone set to ${tone}`,
        `Mode set to ${mode}`,
 ...
 */
function recomposeSentenceAdvanced(sentence, strength, mode, tone) {
    var _a, _b, _c, _d, _e, _f;
    const trimmed = cleanArtifacts(sentence);
    if (!trimmed)
        return trimmed;
    ...
    const firstStart = ((_a = first.split(/\s+/)[0]) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || "";
    const restStart = ((_b = joinedRest.split(/\s+/)[0]) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
    ...
    if (isFormalMode) {
        ...
        if (strength === "medium") {
            return cleanArtifacts(`${capitalize(joinedRest)}. ${capitalize(first)}.`);
        }
        ...
        const longestStart = ((_c = longest.split(/\s+/)[0]) === null || _c === void 0 ? void 0 : _c.toLowerCase()) || "";
        const othersStart = ((_d = others.split(/\s+/)[0]) === null || _d === void 0 ? void 0 : _d.toLowerCase()) || "";
        if (blockedStarts.has(longestStart) || blockedStarts.has(othersStart)) {
            return trimmed;
        }
        return cleanArtifacts(`${capitalize(longest)}. ${capitalize(others)}.`);
    }
    if (strength === "medium") {
    ...
    const longestStart = ((_e = longest.split(/\s+/)[0]) === null || _e === void 0 ? void 0 : _e.toLowerCase()) || "";
    const othersStart = ((_f = others.split(/\s+/)[0]) === null || _f === void 0 ? void 0 : _f.toLowerCase()) || "";
    if (blockedStarts.has(longestStart) || blockedStarts.has(othersStart)) {
        return trimmed;
    }
    return cleanArtifacts(`${capitalize(longest)}. ${capitalize(others)}.`);
}
function longestSharedSpanWords(input, output) {
...
function looksExplanatorySpan(sentence) {
    const cleaned = cleanArtifacts(sentence);
    return (countWords(cleaned) >= 10 &&
        (/,\s*/.test(cleaned) ||
            /\b(because|although|while|which|that|can|could|would|should|may|might|allows|allow|suggests|means|depends|reason|argument|process|policy|program|examples|expected|improve|reduce|support)\b/i.test(cleaned)));
}
function findCopiedExplanatorySpan(inputText, outputText) {
    ...
    for (let sentenceIndex = 0; sentenceIndex < outputSentences.length; sentenceIndex++) {
        ...
        if (!looksExplanatorySpan(outputSentence))
            continue;
        for (const sourceSentence of sourceSentences) {
            ...
            const spanScore = localSharedSpan * 1.8 +
                localSimilarity * 18 +
                Math.min(countWords(outputSentence), 24) * 0.2;
            if (localSharedSpan >= 8 &&
...
function addLocalRewriteVariant(variants, candidate) {
    const cleaned = refineHumanizedOutput(cleanArtifacts(candidate));
    if (cleaned) {
        variants.add(cleaned);
    }
}
function buildOutputOnlyLocalRewriteVariants(sentence, mode, tone, _strength) {
    var _a, _b, _c, _d, _e, _f;
    ...
    addLocalRewriteVariant(variants, recomposeSentenceAdvanced(cleaned, "strong", mode, tone));
    ...
    if (althoughMatch) {
        const main = (_a = althoughMatch[1]) === null || _a === void 0 ? void 0 : _a.trim().replace(/[.!?]+$/, "");
        const support = (_b = althoughMatch[2]) === null || _b === void 0 ? void 0 : _b.trim().replace(/[.!?]+$/, "");
    ...
    if (butMatch) {
        const lead = (_c = butMatch[1]) === null || _c === void 0 ? void 0 : _c.trim().replace(/[.!?]+$/, "");
        const contrast = (_d = butMatch[2]) === null || _d === void 0 ? void 0 : _d.trim().replace(/[.!?]+$/, "");
    ...
    if (becauseMatch) {
        const main = (_e = becauseMatch[1]) === null || _e === void 0 ? void 0 : _e.trim().replace(/[.!?]+$/, "");
        const reason = (_f = becauseMatch[2]) === null || _f === void 0 ? void 0 : _f.trim().replace(/[.!?]+$/, "");
        if (main && reason && countWords(main) >= 7 && countWords(reason) >= 5) {
            ...
            addLocalRewriteVariant(variants, `${capitalize(main)}. This is because ${lowerFirst(reason)}.`);
        }
    }
    ...
    if (/^Rather than focusing only on theory\b/i.test(cleaned)) {
        ...
        addLocalRewriteVariant(variants, cleaned.replace(/^Rather than focusing only on theory,\s*/i, "Instead of relying on theory alone, "));
    }
    ...
    if (/^New hires usually understand the general process\b/i.test(cleaned)) {
        addLocalRewriteVariant(variants, cleaned
            .replace(/^New hires usually understand the general process after reading it once,\s*although\s*/i, "New hires usually understand the general process after one read. But ")
...
function buildSourceAwareLocalRewriteVariants(target, mode, tone, strength) {
    ...
    addLocalRewriteVariant(variants, locallyRecomposeSentence(recomposeSentenceAdvanced(source, "strong", mode, tone), mode, "strong"));
    ...
    const sourceLead = (_a = sourceClauses[0]) === null || _a === void 0 ? void 0 : _a.replace(/[.!?]+$/, "").trim();
    const sourceTail = sourceClauses.slice(1).join(", ").replace(/[.!?]+$/, "").trim();
    const outputLead = (_b = outputClauses[0]) === null || _b === void 0 ? void 0 : _b.replace(/[.!?]+$/, "").trim();
    ...
    if (sourceContrastMatch) {
        const lead = (_c = sourceContrastMatch[1]) === null || _c === void 0 ? void 0 : _c.trim().replace(/[.!?]+$/, "");
        const contrast = (_d = sourceContrastMatch[3]) === null || _d === void 0 ? void 0 : _d.trim().replace(/[.!?]+$/, "");
    ...
    if (sourceBecauseMatch) {
        const main = (_e = sourceBecauseMatch[1]) === null || _e === void 0 ? void 0 : _e.trim().replace(/[.!?]+$/, "");
        const reason = (_f = sourceBecauseMatch[2]) === null || _f === void 0 ? void 0 : _f.trim().replace(/[.!?]+$/, "");
    ...
    if (sourceAlthoughMatch) {
        const main = (_g = sourceAlthoughMatch[1]) === null || _g === void 0 ? void 0 : _g.trim().replace(/[.!?]+$/, "");
        const support = (_h = sourceAlthoughMatch[2]) === null || _h === void 0 ? void 0 : _h.trim().replace(/[.!?]+$/, "");
    ...
    if (/^Rather than focusing only on theory,\s*(.+)$/i.test(source)) {
        ...
        addLocalRewriteVariant(variants, `The program is not limited to theory. ${capitalize(rest)}`);
    }
    if (/^Participants are expected to practice difficult conversations,\s*review examples of unclear delegation,\s*and document how they would respond in realistic situations\.$/i.test(source)) {
        addLocalRewriteVariant(variants, "Participants are expected to do three things: practice difficult conversations, review examples of unclear delegation, and document how they would respond in realistic situations.");
    }
    if (/^Early feedback suggests that the practical format is more useful than longer presentation-style sessions because it allows participants to test ideas and receive immediate input from others\.$/i.test(source)) {
        addLocalRewriteVariant(variants, "Early feedback suggests the practical format works better than longer presentation-style sessions. It gives participants room to test ideas and get immediate input from others.");
    }
    if (/^At the same time,\s*the pilot also showed that time pressure remains a problem for some teams,\s*particularly when workshops are scheduled during busy reporting periods\.$/i.test(source)) {
        addLocalRewriteVariant(variants, "The pilot also showed that time pressure still causes problems for some teams, especially when workshops fall during busy reporting periods.");
    }
    if (/^A better argument for uniforms is that they can simplify daily routines for families and reduce distractions in some school environments,\s*although even that benefit depends on how the policy is applied\.$/i.test(source)) {
        addLocalRewriteVariant(variants, "A stronger case for uniforms is that they can make daily routines easier for families and may reduce distractions in some school environments. Even then, that depends on how the policy is applied.");
    }
    if (/^New hires usually understand the general process after reading it once,\s*although they still have to go back and check details because the examples are not placed where people would expect them\.$/i.test(source)) {
        addLocalRewriteVariant(variants, "Most new hires understand the overall process after one read. They still go back for details, though, because the examples are not placed where people expect them.");
    }
    return unique([...variants].filter(Boolean));
}
function selectBestLocalRewriteVariant(target, candidates) {
    let best = target.outputSentence;
    let bestScore = Number.NEGATIVE_INFINITY;
    for (const candidate of candidates) {
        const cleaned = cleanArtifacts(candidate);
        if (!cleaned || cleaned === target.outputSentence)
            continue;
        ...
        const readabilityPenalty = (/^(Which|Because|Although|And|But|So)\b/.test(cleaned) ? 18 : 0) +
            (countWords(cleaned) < 6 ? 20 : 0);
        if (sourceMeaning < 84 || outputMeaning < 80)
            continue;
        const score = localMeaning -
            sourceSimilarity * 20 -
            localSharedSpan * 1.4 +
            spanGain * 3.5 +
            similarityGain * 16 -
...
function refineLongestCopiedSpan(inputText, outputText, tone, mode, strength, sourceProfile) {
    if (strength === "light" || (sourceProfile === null || sourceProfile === void 0 ? void 0 : sourceProfile.genericAiLike)) {
        return outputText;
    }
    const target = findCopiedExplanatorySpan(inputText, outputText);
    if (!target)
        return outputText;
    const localCandidates = buildSourceAwareLocalRewriteVariants(target, mode, tone, strength);
...
function avoidWeakOpening(sentence) {
...
function rewriteWithSimilarityGuard(inputText, tone, mode, strength) {
    ...
    const secondLooksReasonable = secondWords >= Math.max(6, Math.floor(firstWords * 0.78)) &&
...
function feelsLightlyEditedAiText(inputText, outputText, strength) {
    const similarity = overallSimilarity(inputText, outputText);
    ...
    const similarityLimit = strength === "light" ? 0.86 :
...
function preserveParagraphRhythm(originalText, rewrittenText, paragraphBias = 0) {
    ...
    const sourceSentenceCounts = originalParagraphs.map((paragraph) => splitSentences(paragraph).length);
    ...
    for (let i = 0; i < adjustedCounts.length; i++) {
        ...
        if (remainingSentences <= 0)
            break;
        const target = i === adjustedCounts.length - 1
            ? remainingSentences
            : Math.max(1, Math.min(adjustedCounts[i], remainingSentences - (remainingParagraphs - 1)));
 ...
 */
function rebalanceParagraphs(text, options) {
    var _a, _b, _c, _d, _e;
    const cleaned = cleanArtifacts(text);
    ...
    const originalParagraphCount = (_a = options === null || options === void 0 ? void 0 : options.originalParagraphCount) !== null && _a !== void 0 ? _a : 1;
    const minParagraphs = (_b = options === null || options === void 0 ? void 0 : options.minParagraphs) !== null && _b !== void 0 ? _b : Math.max(1, originalParagraphCount - 1);
    const maxParagraphs = (_c = options === null || options === void 0 ? void 0 : options.maxParagraphs) !== null && _c !== void 0 ? _c : Math.max(minParagraphs, originalParagraphCount + 1);
    const minSentencesPerParagraph = (_d = options === null || options === void 0 ? void 0 : options.minSentencesPerParagraph) !== null && _d !== void 0 ? _d : 2;
    const maxSentencesPerParagraph = (_e = options === null || options === void 0 ? void 0 : options.maxSentencesPerParagraph) !== null && _e !== void 0 ? _e : 4;
    let targetParagraphs = Math.ceil(sentences.length / 3);
    targetParagraphs = Math.max(minParagraphs, Math.min(maxParagraphs, targetParagraphs));
    const paragraphs = [];
    let current = [];
    for (let i = 0; i < sentences.length; i++) {
        ...
        const canCloseParagraph = current.length >= minSentencesPerParagraph &&
 ...
 * - cleanArtifacts(text)
 */
function cleanupSentenceStarters(text) {
    let out = text;
    for (const [pattern, replacement] of starterCleanupLibrary_1.STARTER_CLEANUP_PATTERNS) {
        out = out.replace(pattern, replacement);
    }
    return cleanArtifacts(out);
}
 ...
 */
function rewriteParagraphAware(text, tone, mode, strength, options) {
    const paragraphs = splitParagraphs(text);
    const safeStrength = (options === null || options === void 0 ? void 0 : options.genericAiSafe)
        ? strength === "strong"
            ? "medium"
            : "light"
        : strength;
    const rewrittenParagraphs = paragraphs.map((paragraph) => {
        var _a, _b, _c;
        let out = paragraph;
        const replacementOptions = (options === null || options === void 0 ? void 0 : options.genericAiSafe) || tone === "formal"
            ? {
                phraseBoost: Math.min((_a = options === null || options === void 0 ? void 0 : options.phraseBoost) !== null && _a !== void 0 ? _a : 0, 0),
                wordBoost: -4,
            }
            : options;
        out = (options === null || options === void 0 ? void 0 : options.genericAiSafe) ? softenGenericAiScaffold(out) : stripAiPhrases(out);
        if (!(options === null || options === void 0 ? void 0 : options.genericAiSafe)) {
            out = applyWordReplacements(out, safeStrength, replacementOptions);
        }
        out = applyParagraphStrategy(out, (_b = options === null || options === void 0 ? void 0 : options.strategy) !== null && _b !== void 0 ? _b : "baseline", safeStrength, mode, tone, (_c = options === null || options === void 0 ? void 0 : options.reorderLevel) !== null && _c !== void 0 ? _c : 0);
 ...
 */
function finalSentencePolish(text) {
    let out = cleanArtifacts(text);
    out = out
        .replace(/\b(I learned[^.]*?from this group project)\.\s+This showed me that\b/gi, "$1 because it showed me that ")
        .replace(/\b(School uniforms are often defended as a way to create equality)\.\s+That argument\b/gi, "$1, but that argument")
        .replace(/\b(This (?:additional|extra) step may appear minor)\.\s+It is intended to\b/gi, "$1, but it is intended to")
        .replace(/\b(Participants are expected to practice difficult conversations,\s*review examples of unclear delegation)\.\s+Document how they would respond in realistic situations\./gi, "$1, and document how they would respond in realistic situations.")
        .replace(/\bin some school environments\.,\s*although\b/gi, "in some school environments, although")
        .replace(/\bin some school environments\.\s+Reduce distractions\b/gi, "in some school environments, reduce distractions")
        .replace(/\bpresentation-style sessions\.\s+Since it allows\b/gi, "presentation-style sessions because it allows")
        .replace(/\bproblem for some teams,\s+This is especially true when\b/gi, "problem for some teams, especially when")
        .replace(/\bproblem for some teams\.\s+This is especially true when\b/gi, "problem for some teams, especially when")
        ...
        .replace(/\bIncluding education, healthcare, business, and counseling, psychology is used in many areas\b/gi, "Psychology is used in many areas, including education, healthcare, business, and counseling")
 ...
 */
function getRewriteProfiles(strength) {
    if (strength === "light") {
        return [
            {
                id: "balanced_a",
 ...
 * - countWords(text)
 */
function scoreRewriteCandidate(inputText, outputText, strength = "medium", sourceProfile) {
    const similarity = overallSimilarity(inputText, outputText);
    ...
    const awkwardStarts = sentences.filter((s) => /^(Which|Because|Although|And|But|So)\b/.test(s.trim())).length;
    ...
    const naturalness = Math.max(0, readability) -
        aiPhraseHits * 8 -
        paragraphPenalty * 6;
    const structuralFreshness = Math.max(0, variation) -
        Math.round(openerMatch * 22);
    const penalties = shortFragments * 10 +
        awkwardStarts * 8 +
        repeatedStarts * 5 +
        aiPhraseHits * 7 +
        paragraphPenalty * 5;
    const meaningThreshold = (sourceProfile === null || sourceProfile === void 0 ? void 0 : sourceProfile.genericAiLike)
        ? strength === "strong"
            ? 78
            : 78
        : strength === "strong"
    ...
    const classicStructureSafe = similarity <= (strength === "light" ? 0.82 : strength === "medium" ? 0.72 : 0.66) &&
        openerMatch <= (strength === "light" ? 0.75 : 0.6) &&
        longestSpan < (strength === "light" ? 12 : strength === "medium" ? 11 : 10);
    const reorderedStructureSafe = strength !== "light" &&
        ...
        openerMatch <= 0.75 &&
        longestSpan < 13;
    const compactReorderedStructureSafe = strength !== "light" &&
        !!sourceProfile &&
        ...
        longestSpan < 28;
    const shortSwapStructureSafe = strength === "strong" &&
        !!sourceProfile &&
        ...
        longestSpan < 28;
    const structureSafe = meaningSafe && (classicStructureSafe ||
        reorderedStructureSafe ||
        compactReorderedStructureSafe ||
        shortSwapStructureSafe);
    const meaningWeight = strength === "light" ? 0.62 : strength === "medium" ? 0.56 : 0.58;
    const naturalnessWeight = strength === "light" ? 0.25 : strength === "medium" ? 0.18 : 0.12;
    const structureWeight = strength === "light" ? 0.13 : strength === "medium" ? 0.26 : 0.3;
    const total = (meaningPreservation * meaningWeight) +
 ...
 */
function generateRewriteCandidates(inputText, tone, mode, strength) {
    const originalParagraphCount = countParagraphs(inputText);
    const sourceProfile = analyzeSourceProfile(inputText);
    ...
    return profiles.map((profile) => {
        ...
        let output = rewriteParagraphAware(inputText, tone, mode, profile.strength, {
            ...
            reorderLevel: genericAiSafe
                ? Math.min(profile.reorderLevel, 1)
        ...
        output = refineLongestCopiedSpan(inputText, output, tone, mode, profile.strength, sourceProfile);
 ...
 */
function pickBestRewriteCandidate(candidates, strength = "medium") {
    ...
    const structureSafeCandidates = meaningSafeCandidates.filter((candidate) => candidate.score.structureSafe);
    const ranked = structureSafeCandidates.length > 0
    ...
    if (strength === "strong") {
        const strongerStructured = ranked.find((candidate) => candidate.profileStrength === "strong" &&
...
function targetedRestructureRetry(inputText, currentOutputText, tone, mode, strength) {
    const originalParagraphCount = countParagraphs(inputText);
    let retry = applyParagraphStrategy(currentOutputText, strength === "strong" ? "structured_strong" : "reordered_emphasis", "strong", mode, tone, strength === "strong" ? 2 : 1);
    retry = aggressiveSecondPass(retry, mode, tone);
    retry = reshapeParagraphStructure(retry, "strong", mode, tone, strength === "strong" ? 2 : 1);
...
function preservationFocusedRetry(inputText, tone, mode, strength) {
    const originalParagraphCount = countParagraphs(inputText);
    let retry = rewriteParagraphAware(inputText, tone, mode, strength === "strong" ? "medium" : strength, {
        phraseBoost: 0,
        wordBoost: 0,
 ...
 * - countParagraphs(text)
 */
function humanizeTextRetry(inputText, currentOutputText, tone = "natural", mode = "standard") {
    const sourceProfile = analyzeSourceProfile(inputText);
    const originalParagraphCount = countParagraphs(inputText);
    let retry = inputText;
    ...
    retry = reshapeParagraphStructure(retry, "strong", mode, tone, sourceProfile.genericAiLike ? 1 : 2);
    ...
    const currentScore = scoreRewriteCandidate(inputText, currentOutputText, "medium", sourceProfile);
    const retryScore = scoreRewriteCandidate(inputText, retry, "medium", sourceProfile);
    if (retryScore.total > currentScore.total) {
        return retry;
    }
    return currentOutputText;
}
function evaluateHumanizeCandidate(inputText, outputText) {
    return scoreRewriteCandidate(inputText, outputText, "medium", analyzeSourceProfile(inputText));
}
